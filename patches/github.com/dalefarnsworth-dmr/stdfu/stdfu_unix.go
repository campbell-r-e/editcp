...
func New() (*StDfu, error) {
	...
	// libusb IOKit init conflicts with Qt's CFRunLoop on the main OS thread,
	// so we initialize USB on a dedicated OS thread.
	go func() {
		...
		func() {
			...
			if err != nil {
				stDfu.Close()
				r.err = fmt.Errorf("OpenDevice failed: %v", err)
				return
			}
			if dev == nil {
				stDfu.Close()
				r.err = fmt.Errorf("No Radio was found on USB")
				return
			}
			stDfu.dev = dev
			...
			if err != nil {
				stDfu.Close()
				r.err = fmt.Errorf("The radio was found on USB, but is not accessible.  Is the radio's USB interface already in use?  Possibly by a Virtual Machine?")
				return
			}
			stDfu.iface = iface
